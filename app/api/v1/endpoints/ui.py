...
async def render_production_page(request: Request):
    return _render(request, "production_upload.html")


@router.post("/production/process", response_class=HTMLResponse)
async def process_production(
    ...
    try:
        ...
        logger.info("Processing production via UI", extra={"company": company, "upload_filename": file.filename})
        result = await run_in_threadpool(process_production_and_update_db, file, company)
        return _render(request, "production_upload.html", success=result.message, warnings=result.warnings)

    except ExcelValidationError as e:
        logger.warning("Validation failed on production via UI", extra={"company": company, "errors": [i.message for i in e.issues]})
        return _render(request, "production_upload.html", validation=[i.message for i in e.issues])
    except ValueError as e:
        logger.warning("Bad production upload via UI", extra={"company": company, "error": str(e)})
        return _render(request, "production_upload.html", validation=[str(e)])
    except Exception:
        logger.exception("Error processing production via UI", extra={"company": company})
        return _render(request, "production_upload.html", error="An unexpected error occurred. Please try again or contact support.")
